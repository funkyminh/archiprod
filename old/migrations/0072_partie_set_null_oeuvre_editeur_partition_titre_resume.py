...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        cursor1.execute("UPDATE partie SET oeuvre_id = NULL WHERE oeuvre_id = 0")
        transaction.commit_unless_managed()

        from django.db import connection, transaction
        cursor2 = connection.cursor()
        cursor2.execute("UPDATE partie SET editeur_partition_id = NULL WHERE editeur_partition_id = 0")
        transaction.commit_unless_managed()

        from django.db import connection, transaction
        cursor3 = connection.cursor()
        cursor3.execute("UPDATE partie SET titre = NULL WHERE titre = ''")
        transaction.commit_unless_managed()

        from django.db import connection, transaction
        cursor4 = connection.cursor()
        cursor4.execute("UPDATE partie SET resume = NULL WHERE resume = ''")
        transaction.commit_unless_managed()

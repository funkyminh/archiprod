...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        cursor.execute("UPDATE video_volume INNER JOIN video_volume2 AS vv2 ON video_volume.id = vv2.id SET video_volume.evenement2_id = vv2.parent")
        transaction.commit_unless_managed()

        '''
        cursor1 = connection.cursor()
        cursor1.execute("UPDATE volume INNER JOIN volume2 AS v2 ON volume.id = v2.id SET volume.evenement2_id = v2.parent")
        transaction.commit_unless_managed()

        cursor2 = connection.cursor()
        cursor2.execute("UPDATE note_programme INNER JOIN note_programme2 AS np2 ON note_programme.id = np2.id SET note_programme.evenement2_id = np2.parent")
        transaction.commit_unless_managed()
        '''

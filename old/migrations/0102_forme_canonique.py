...
#tables concernées : personne, role, collectivite
#attention aux tables : oeuvre_personne_role, video_record_collectivite, video_record_personne_role, video_volume_collectivite, video_volume_personne_role, volume_collectivite, volume_personne_role
#used : oeuvre_personne_role
#not used : video_record_collectivite, video_record_personne_role, video_volume_collectivite, video_volume_collectivite, video_volume_personne_role, volume_collectivite, volume_personne_role

UPDATE
    oeuvre_personne_role as opr
INNER JOIN
    personne as p
ON
    p.`id` = opr.`personne_id`
SET
    opr.personne_id = p.forme_canonique_id
WHERE p.indice = '-1';
...
ON
    r.`id` = opr.`role_id`
SET
    opr.role_id = r.forme_canonique_id
WHERE r.indice = '-1';
        '''

        from django.db import connection, transaction
        cursor1 = connection.cursor()
        cursor1.execute("UPDATE oeuvre_personne_role as opr INNER JOIN personne as p ON p.`id` = opr.`personne_id` SET opr.`personne_id` = p.`forme_canonique_id` WHERE p.`indice` = '-1'")
        transaction.commit_unless_managed()

        from django.db import connection, transaction
        cursor2 = connection.cursor()
        cursor2.execute("UPDATE oeuvre_personne_role as opr INNER JOIN role as r ON r.`id` = opr.`role_id` SET opr.`role_id` = r.`forme_canonique_id` WHERE r.`indice` = '-1'")
        transaction.commit_unless_managed()

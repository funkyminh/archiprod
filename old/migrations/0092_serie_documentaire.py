...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        cursor1.execute("UPDATE evenement SET video_children = NULL")
        transaction.commit_unless_managed()

        from django.db import connection, transaction
        cursor2 = connection.cursor()
        cursor2.execute("INSERT INTO evenement(id, intitule, video_children, supprime, sous_titre, parent_id, date_debut, date_fin, time_stamp, type_id, event_children, audio_children, pdf_children, flag_archive, flag_categorie, lft, rght, tree_id, level) SELECT NULL, intitule, id, supprime, sous_titre, parent_id, date_debut, date_fin, time_stamp, NULL, NULL, NULL, NULL, 0, 1, 0, 0, 0, 1 from video_set")
        transaction.commit_unless_managed()

        from django.db import connection, transaction
        cursor3 = connection.cursor()
        cursor3.execute("UPDATE video_volume INNER JOIN evenement AS ev ON video_volume.parent_id = ev.video_children SET video_volume.parent_id = ev.id, video_volume.saison_id = NULL WHERE video_volume.type = 2")
        transaction.commit_unless_managed()

...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        cursor1.execute("UPDATE video_volume INNER JOIN evenement ON video_volume.`parent_id` = evenement.`id` SET video_volume.`intitule` = evenement.`intitule`, video_volume.`sous_titre` = evenement.`sous_titre`, video_volume.`type_evenement_id` = evenement.`type_id` WHERE video_volume.`type` != 2")
        transaction.commit_unless_managed()

        cursor2 = connection.cursor()
        cursor2.execute("UPDATE volume INNER JOIN evenement ON volume.`parent_id` = evenement.`id` SET volume.`intitule` = evenement.`intitule`, volume.`sous_titre` = evenement.`sous_titre`, volume.`type_evenement_id` = evenement.`type_id`")
        transaction.commit_unless_managed()

        cursor = connection.cursor()
        cursor.execute("ALTER TABLE note_programme DROP index `intitule`")
        transaction.commit_unless_managed()

        cursor3 = connection.cursor()
        cursor3.execute("UPDATE note_programme INNER JOIN evenement ON note_programme.`parent_id` = evenement.`id` SET note_programme.`intitule` = evenement.`intitule`, note_programme.`sous_titre` = evenement.`sous_titre`, note_programme.`type_evenement_id` = evenement.`type_id` WHERE note_programme.`parent_id` != 997")
        #id=997 correspond evenement fictif
        transaction.commit_unless_managed()

        cursor4 = connection.cursor()
        cursor4.execute("UPDATE evenement INNER JOIN video_volume ON video_volume.`parent_id` = evenement.`id` SET evenement.`flag_archive` = true WHERE video_volume.`type` != 2")
        transaction.commit_unless_managed()

        cursor5 = connection.cursor()
        cursor5.execute("UPDATE evenement INNER JOIN volume ON volume.`parent_id` = evenement.`id` SET evenement.`flag_archive` = true")
        transaction.commit_unless_managed()

        cursor6 = connection.cursor()
        cursor6.execute("UPDATE evenement INNER JOIN note_programme ON note_programme.`parent_id` = evenement.`id` SET evenement.`flag_archive` = true")
        transaction.commit_unless_managed()

...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        cursor1.execute("UPDATE video_volume SET notes = NULL WHERE notes = ''")
        transaction.commit_unless_managed()
        
        from django.db import connection, transaction
        cursor2 = connection.cursor()
        cursor2.execute("UPDATE video_volume SET resume = NULL WHERE resume = ''")
        transaction.commit_unless_managed()

        from django.db import connection, transaction
        cursor3 = connection.cursor()
        cursor3.execute("UPDATE video_volume SET intitule = NULL WHERE intitule = ''")
        transaction.commit_unless_managed()
        
        from django.db import connection, transaction
        cursor4 = connection.cursor()
        cursor4.execute("UPDATE video_volume SET parent_id = NULL WHERE parent_id = 0")
        transaction.commit_unless_managed()

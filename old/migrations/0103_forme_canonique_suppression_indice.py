...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        cursor1.execute("DELETE FROM role WHERE indice = '-1'")
        transaction.commit_unless_managed()

        cursor2 = connection.cursor()
        cursor2.execute("DELETE FROM personne WHERE indice = '-1'")
        transaction.commit_unless_managed()

        cursor2 = connection.cursor()
        cursor2.execute("DELETE FROM collectivite WHERE indice = '-1'")
        transaction.commit_unless_managed()

...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        '''
        from old.models import Evenement #, NoteProgramme
        video_volumes = orm.NoteProgramme.objects.all().exclude(parent_id=None)
        for video_volume in video_volumes:
            evt = Evenement.objects.get(id=video_volume.parent_id)
            try:
                new_evt = evt.get_ancestors(include_self=True).get(flag_categorie=1)
                video_volume.parent = new_evt
                video_volume.save()
            except:
                print evt.id
        '''
        ...
        for video_volume in video_volumes:
            ...
            tree_id = evt.tree_id
            try:
                new_evt = orm.Evenement.objects.filter(lft__lte=lft, rght__gte=rght, tree_id=tree_id)\
                .exclude(level=0).exclude(id=evt.id).get(flag_categorie=1)
                video_volume.parent = new_evt
                video_volume.save()
            except:
                print evt.id

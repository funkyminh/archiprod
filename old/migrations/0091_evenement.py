...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        cursor1.execute("UPDATE evenement SET `flag_categorie` = TRUE WHERE level = 1")
        transaction.commit_unless_managed()

        cursor1 = connection.cursor()
        cursor1.execute("UPDATE evenement SET `flag_categorie` = TRUE WHERE id = 997")
        transaction.commit_unless_managed()

        from django.db import connection, transaction
        cursor2 = connection.cursor()
        cursor2.execute("INSERT IGNORE INTO tag_evenement SELECT ev1.`id`,ev1.`intitule`,ev1.`sous_titre`,ev1.`date_debut`,ev1.`date_fin`,ev1.`time_stamp` FROM evenement as ev INNER JOIN evenement as ev1 ON ev.`parent_id` = ev1.`id` WHERE ev1.`flag_categorie` = FALSE AND ev.`flag_archive` = TRUE")
        transaction.commit_unless_managed()

        from django.db import connection, transaction
        cursor3 = connection.cursor()
        cursor3.execute("UPDATE video_volume INNER JOIN evenement as ev ON video_volume.`parent_id` = ev.`id` INNER JOIN evenement as ev1 ON ev.`parent_id` = ev1.`id` SET video_volume.`tag_evenement_id` = ev1.`id` WHERE ev1.`flag_categorie` = FALSE")
        transaction.commit_unless_managed()

        from django.db import connection, transaction
        cursor4 = connection.cursor()
        cursor4.execute("UPDATE volume INNER JOIN evenement as ev ON volume.`parent_id` = ev.`id` INNER JOIN evenement as ev1 ON ev.`parent_id` = ev1.`id` SET volume.`tag_evenement_id` = ev1.`id` WHERE ev1.`flag_categorie` = FALSE")
        transaction.commit_unless_managed()

        from django.db import connection, transaction
        cursor5 = connection.cursor()
        cursor5.execute("UPDATE note_programme INNER JOIN evenement as ev ON note_programme.`parent_id` = ev.`id` INNER JOIN evenement as ev1 ON ev.`parent_id` = ev1.`id` SET note_programme.`tag_evenement_id` = ev1.`id` WHERE ev1.`flag_categorie` = FALSE")
        transaction.commit_unless_managed()

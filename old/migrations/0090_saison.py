...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        cursor1.execute("INSERT INTO saison SELECT id,intitule,sous_titre,date_debut,date_fin,time_stamp FROM evenement WHERE evenement.`parent_id` is NULL")
        transaction.commit_unless_managed()

        from django.db import connection, transaction
        cursor2 = connection.cursor()
        cursor2.execute("UPDATE `video_volume` inner join evenement on video_volume.`parent_id` = evenement.`id` SET saison_id = (SELECT ev.`id` FROM evenement AS ev WHERE ev.`tree_id` = evenement.`tree_id` AND ev.`level` = 0)")
        transaction.commit_unless_managed()

        from django.db import connection, transaction
        cursor3 = connection.cursor()
        cursor3.execute("UPDATE `volume` inner join evenement on volume.`parent_id` = evenement.`id` SET saison_id = (SELECT ev.`id` FROM evenement AS ev WHERE ev.`tree_id` = evenement.`tree_id` AND ev.`level` = 0)")
        transaction.commit_unless_managed()

        from django.db import connection, transaction
        cursor4 = connection.cursor()
        cursor4.execute("UPDATE `note_programme` inner join evenement on note_programme.`parent_id` = evenement.`id` SET saison_id = (SELECT ev.`id` FROM evenement AS ev WHERE ev.`tree_id` = evenement.`tree_id` AND ev.`level` = 0)")
        transaction.commit_unless_managed()

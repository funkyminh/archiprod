...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for media in orm['archives.Media'].objects.all().filter(media__startswith='AU'):
            if len(media.media) == 11:
                media_ID = '%s-0%s' % (media.media[:-1], media.order)
            if len(media.media) == 12:
                media_ID = '%s-%s' % (media.media[:-2], media.order)
            cursor1 = connection.cursor()
            cursor1.execute("select `partie_id`,`personne_id`,`role_id`, `id` from `partie_personne_role` where `partie_id` = '%s'" % (media_ID))
            rows = cursor1.fetchall()
            for row in rows:
                person = orm['utils.Person'].objects.get(id=row[1])
                role = orm['utils.Role'].objects.get(id=row[2])
                orm['archives.Participant'](person=person, role=role, media=media).save()
    ...
    models = {
        ...
        'archives.media': {
            ...
            'time_stamp': ('django.db.models.fields.DateTimeField', [], {'auto_now': 'True', 'blank': 'True'}),
            'title': ('django.db.models.fields.CharField', [], {'max_length': '255', 'null': 'True', 'blank': 'True'}),
            'type_work': ('django.db.models.fields.CharField', [], {'max_length': '20', 'null': 'True', 'blank': 'True'}),
            'user': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['auth.User']", 'null': 'True', 'blank': 'True'}),

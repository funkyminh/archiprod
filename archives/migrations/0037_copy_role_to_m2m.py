...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for item in orm['archives.ArchiveParticipant'].objects.all():
            print item, item.id
            item.role_new.add(item.role)
            item.save()
        for item in orm['archives.Participant'].objects.all():
            print item, item.id
            item.role_new.add(item.role)
            item.save()
        for item in orm['archives.ArchiveCollectivity'].objects.all():
            print item, item.id
            if item.role:
                item.role_new.add(item.role)
                item.save()
        for item in orm['archives.MediaCollectivity'].objects.all():
            print item, item.id
            if item.role:
                item.role_new.add(item.role)
                item.save()

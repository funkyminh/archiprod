...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for archive in orm['archives.Archive'].objects.all().filter(id_archiprod__startswith='AU'):
            archive_ID = archive.id_archiprod
            cursor1 = connection.cursor()
            cursor1.execute("select `volume_id`,`personne_id`,`role_id` from `volume_personne_role` where `volume_id` = '%s'" % (archive_ID))
            rows = cursor1.fetchall()
            for row in rows:
                person = orm['utils.Person'].objects.get(id=row[1])
                role = orm['utils.Role'].objects.get(id=row[2])
                orm['archives.ArchiveParticipant'](person=person, role=role, archive=archive).save()

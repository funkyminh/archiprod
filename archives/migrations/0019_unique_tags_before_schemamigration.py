...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        tags = orm['archives.Tag'].objects.raw("select *, count(*) from archives_tag group by label, comment having count(*) > 1 ")
        for tag in tags:
            current_tags = orm['archives.Tag'].objects.filter(label=tag.label, comment=tag.comment)
            #print current_tags
            archives = orm['archives.Archive'].objects.filter(tags__in=current_tags)
            #print archives
            for archive in archives:
                archive.tags.add(current_tags[0])
            for ctag in current_tags[1:]:
                ctag.delete()
